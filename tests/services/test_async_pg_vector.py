...
@pytest.mark.asyncio
async def test_get_file_ids_passes_file_ids(store):
    with patch.object(
        ExtendedPgVector, "get_file_ids", return_value=["file1"]
    ) as mock:
        result = await store.get_file_ids(["file1", "file2"])
    mock.assert_called_once_with(["file1", "file2"])
    assert result == ["file1"]


@pytest.mark.asyncio
async def test_get_ids_by_file_ids_passes_file_ids(store):
    with patch.object(
        ExtendedPgVector, "get_ids_by_file_ids", return_value=["chunk1"]
    ) as mock:
        result = await store.get_ids_by_file_ids(["file1"])
    mock.assert_called_once_with(["file1"])
...
@pytest.mark.asyncio
async def test_delete_by_file_ids_passes_file_ids(store):
    with patch.object(ExtendedPgVector, "delete_by_file_ids") as mock:
        await store.delete_by_file_ids(["file1"])
    mock.assert_called_once_with(["file1"])
