...
def test_extended_pgvector_delete_by_file_ids_deletes_chunk_ids(monkeypatch):
    dummy_vector = DummyPgVector()
    deleted = {}

    monkeypatch.setattr(
        dummy_vector, "get_ids_by_file_ids", lambda file_ids: ["chunk1", "chunk2"]
    )
    monkeypatch.setattr(
        dummy_vector, "_delete_multiple", lambda ids: deleted.update({"ids": ids})
    )

    dummy_vector.delete_by_file_ids(["file1"])

    assert deleted["ids"] == ["chunk1", "chunk2"]
...
class TestHandleFieldFilter:
    ...
    def test_like_uses_astext_not_jsonb_path_match(self, store):
        clause = store._handle_field_filter("file_id", {"$like": "hotpotqa-day11-%"})
        sql = _compile(clause)
        assert "->>" in sql
        assert "LIKE" in sql
        assert "jsonb_path_match" not in sql

    def test_ilike_uses_astext_not_jsonb_path_match(self, store):
        clause = store._handle_field_filter("file_id", {"$ilike": "hotpotqa-day11-%"})
        sql = _compile(clause)
        assert "->>" in sql
        assert "ILIKE" in sql
        assert "jsonb_path_match" not in sql
