...
class TestBatchProcessingResilience:
    ...
    @pytest.mark.asyncio
    async def test_rollback_called_with_inserted_chunk_ids(self):
        """Test that rollback deletes the chunk IDs that were actually inserted."""
        from app.routes.document_routes import _process_documents_async_pipeline
        ...
        # Verify delete was called with the chunk IDs returned by the successful batch.
        mock_store.delete.assert_called_once()
        call_kwargs = mock_store.delete.call_args
        assert call_kwargs[1]["ids"] == ["id1"]
