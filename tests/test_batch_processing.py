...
class TestBatchProcessing:
    ...
    def mock_documents(self):
        ...
        return [
            Document(
                ...
                metadata={"file_id": "test_file", "chunk_id": f"chunk_{i}"},
            )
            for i in range(10)
        ]
    ...
    async def test_async_pipeline_basic(self, mock_documents, mock_async_vector_store):
        ...
        first_call = mock_async_vector_store.aadd_documents.call_args_list[0]
        assert first_call.kwargs["ids"] == ["chunk_0", "chunk_1", "chunk_2"]
    ...
    async def test_sync_batched_basic(self, mock_documents, mock_sync_vector_store):
        ...
        first_call = mock_sync_vector_store.add_documents.call_args_list[0]
        assert first_call.kwargs["ids"] == ["chunk_0", "chunk_1", "chunk_2"]
