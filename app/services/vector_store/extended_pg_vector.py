...
class ExtendedPgVector(PGVector):
    ...
    def _handle_field_filter(self, field: str, value: Any) -> Any:
        ...
        This override rewrites $eq, $ne, $like, and $ilike to use the ->>' astext
        operator instead, producing index-friendly predicates such as
        WHERE (cmetadata->>'field') = 'value'. All other operators ($lt, $gt,
        $in, $between, etc.) delegate to the parent.
        """
        ...
        if operator == "$eq":
            ...
            return self.EmbeddingStore.cmetadata[field].astext.like(
                str(filter_value), escape="\\"
            )
        elif operator == "$ilike":
            return self.EmbeddingStore.cmetadata[field].astext.ilike(
                str(filter_value), escape="\\"
            )
    ...
    def get_file_ids(self, file_ids: list[str]) -> list[str]:
        with Session(self._bind) as session:
            file_id_field = self.EmbeddingStore.cmetadata["file_id"].astext
            query = (
                session.query(file_id_field)
                .filter(file_id_field.in_([str(file_id) for file_id in file_ids]))
                .distinct()
            )
            results = query.all()
            return [result[0] for result in results if result[0] is not None]
    ...
    def get_ids_by_file_ids(self, file_ids: list[str]) -> list[str]:
        with Session(self._bind) as session:
            file_id_field = self.EmbeddingStore.cmetadata["file_id"].astext
            query = session.query(self.EmbeddingStore.custom_id).filter(
                file_id_field.in_([str(file_id) for file_id in file_ids])
            )
            results = query.all()
            return [result[0] for result in results if result[0] is not None]

    def delete_by_file_ids(self, file_ids: list[str]) -> None:
        chunk_ids = self.get_ids_by_file_ids(file_ids)
        if chunk_ids:
            self._delete_multiple(ids=chunk_ids)
