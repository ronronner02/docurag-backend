...
class AsyncPgVector(ExtendedPgVector):
    ...
    async def get_file_ids(self, file_ids: list[str], executor=None) -> list[str]:
        executor = executor or self._get_thread_pool()
        return await self._run_in_executor(executor, super().get_file_ids, file_ids)

    async def get_ids_by_file_ids(
        self, file_ids: list[str], executor=None
    ) -> list[str]:
        executor = executor or self._get_thread_pool()
        return await self._run_in_executor(
            executor, super().get_ids_by_file_ids, file_ids
        )

    async def delete_by_file_ids(self, file_ids: list[str], executor=None) -> None:
        executor = executor or self._get_thread_pool()
        await self._run_in_executor(executor, super().delete_by_file_ids, file_ids)
